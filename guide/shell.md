...
### Workspace home: `/home/<name>` and `~`

**Status:** ✅ Implemented · **Category:** filesystem · **Since:** 2026-05-04

Each workspace appears in the shell view as a real Linux-style user
home at `/home/<sanitized-workspace-name>`. The default workspace
lives at `/home/default`. The active workspace's `$HOME` is exported
to that path; the path is also reachable via `~` (tilde) as before.
...
`/home/<active>/foo` and `~/foo` resolve to the same OPFS location.
Storage layout under the hood is unchanged: each workspace's data
still lives at `clawser/workspaces/{wsId}/...`, and the `/home/<name>`
view is a path-resolution alias over that.

#### Workspace name → directory name

The user-facing display name (e.g. `"My Project"`) is sanitized into
a filename-safe form for the `/home/<name>` slot:

- Lowercased, NFKD-normalized.
- Non-ASCII characters dropped (`"Café"` → `"cafe"`).
- Whitespace and other separators collapse to a single dash.
- Reserved names rejected (`proc`, `etc`, `dev`, `tmp`, `home`,
  `sys`, `.`, `..`).
- Collisions resolved with stable numeric suffixes
  (`my-project` → `my-project-2` → `my-project-3`).
- The default workspace always claims the bare `default` name.

```
$ cat /proc/clawser/workspaces
id            name           home              active
default       workspace      /home/default     yes
ws_lk2tnk     My Project     /home/my-project
ws_lk2to8     Café           /home/cafe
ws_lk2tp4     My  Project    /home/my-project-2
```

#### Workspace switch — live `$HOME`

When you switch workspaces (UI dropdown or `clawser workspace switch`),
the shell session is rebuilt with `$HOME` set to the new
`/home/<name>`. Profiles re-source against the new home. A terminal
that was sitting at `~/scripts` ends up at the new workspace's
`/scripts` (interpreted relative to the new root).

#### Cross-workspace isolation

Workspaces are fully isolated in the shell view. Reading or writing
`/home/<other>/anything` fails:

```
$ ls /home/their-project/
ls: /home/their-project/: No such file or directory

$ echo data > /home/their-project/secret.txt
redirect: cross-workspace write denied: /home/their-project/secret.txt (workspaces are isolated)
```

Internally these paths route to a never-created `_isolated_/` OPFS
subtree, so reads naturally ENOENT and writes are explicitly rejected
at the shell-redirect layer (and again at `ShellFs.#guardWrite` for
defense in depth). To inspect another workspace's files, switch to
it via the workspace dropdown.

`/proc/clawser/workspaces` is the one place where you can see all
workspace names + their `/home/<name>` paths regardless of which one
is active.
