...
3. Recovery (forgotten passphrase) was impossible without backup codes,
   which we'd decided not to ship.
...
coordination. It is created lazily on the first `addPasskeyWrap` call.
...
**Recovery codes.** A printable code that could re-derive a wrap KEK
sounds nice but is structurally redundant with passkeys: it's just a
weaker, copy-pastable unlock path that users will lose track of. In a
browser-local model the user already has `Reset all data` (in the same
settings panel) for the case where they've lost every unlock material —
there's nothing on a remote server we could use to restore.

For users who want a backup, the vault settings panel exposes
`Export vault backup`, which emits a JSON file containing the encrypted
blobs. Without a working unlock path the export is useless to anyone
else; restore it onto a new device and unlock with the original
