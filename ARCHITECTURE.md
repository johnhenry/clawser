...
The Codex uses **andbox** (`andbox` npm package, loaded via CDN) — a Worker-based sandbox with RPC capabilities, import maps, timeouts, and capability gating. Browser tools are exposed as host capabilities callable via `host.call()`, with `print()` and `fetch()` injected as local functions.
...
Clawser is a **Pod** — any browser execution context that can execute code, receive messages, and be discovered/addressed. The Pod base class (`browsermesh-pod` npm package) implements a 6-phase boot sequence with zero Clawser dependencies, including PBFT consensus integration and pluggable transport/discovery adapters.

```
Pod (base class — zero Clawser deps)
  ├── ClawserPod    — full agent workspace (agent, tools, UI, gateway, mesh)
  ├── InjectedPod   — lightweight, injected into arbitrary pages via extension
  └── EmbeddedPod   — developer API for embedding pods into apps
```
...
| Phase | Name | Action |
|-------|------|--------|
| 0 | Install Runtime | Generate Ed25519 identity, detect kind & capabilities, attach to `globalThis[Symbol.for('pod.runtime')]` |
| 1 | Install Listeners | `window.message` handler + subclass hook `_onInstallListeners()` |
| 2 | Self-Classification | Detect parent/opener/SW relationships |
| 3 | Parent Handshake | Send `POD_HELLO` to parent, wait for `POD_HELLO_ACK` (1s timeout) |
| 4 | Peer Discovery | Announce on `BroadcastChannel('pod-discovery')`, collect responses (2s timeout) |
| 5 | Role Finalization | Determine role (autonomous/child/peer), call `_onReady()`, emit `'ready'` |

### Pod Kinds

`detectPodKind(globalThis)` classifies the execution context: `service-worker`, `shared-worker`, `worker`, `worklet`, `server`, `iframe`, `spawned`, or `window`.

### Discovery

Same-origin pods discover each other automatically via BroadcastChannel. Cross-origin pods use the extension relay or a relay server. The wire protocol uses `POD_HELLO`, `POD_HELLO_ACK`, `POD_GOODBYE`, `POD_MESSAGE`, `POD_RPC_REQUEST`, and `POD_RPC_RESPONSE` message types.

### ClawserPod

`ClawserPod` (`web/clawser-pod.js`) extends Pod and adds mesh networking via `initMesh()`:

```
ClawserPod.boot()       → Pod identity + discovery
ClawserPod.initMesh()   → MeshIdentityManager + IdentityWallet + PeerRegistry
                           + PeerNode + SwarmCoordinator
```

`initMeshSubsystem()` in workspace lifecycle creates a `ClawserPod`, boots it, then layers on the full mesh stack.

### Extension Injection

The Chrome extension ([Clawser Browser Control](https://chromewebstore.google.com/detail/clawser-browser-control/dljchbfodafekojicopaboiegophjcbc), source in the `clawser-browser-control` repo) can inject an `InjectedPod` into any page via `inject_pod` action. It guards against double-injection via `Symbol.for('pod.runtime')` and shows a visual overlay indicator.

Cross-origin pod discovery and relay is handled by the signaling and relay servers deployed at `wss://browsermesh-signaling.fly.dev` and `wss://browsermesh-relay.fly.dev` (source in the `browsermesh-servers` repo).
...
index.html
  └─ clawser-app.js
       ...
       ├─ clawser-pod.js (ClawserPod — Pod + mesh networking)
       │    └─ browsermesh-pod (npm — Pod base class, discovery, messaging)
       ├─ clawser-embed.js (EmbeddedPod — embeddable pod)
...
graph TD
    ...
    APP --> POD["clawser-pod.js (ClawserPod)"]
    APP --> FEATURES["30+ feature modules"]

    POD --> POD_PKG["browsermesh-pod (npm)"]
    POD --> MESH["browsermesh-primitives + netway (npm)"]
