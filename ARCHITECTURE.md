...
Clawser is a **Pod** — any browser execution context that can execute code, receive messages, and be discovered/addressed. The Pod base class (`web/packages/pod/`) implements a 6-phase boot sequence with zero Clawser dependencies.
...
The Chrome extension can inject an `InjectedPod` into any page via `inject_pod` action. The injection script (`extension/pod-inject.js`) is a concatenated IIFE generated by `web/packages/pod/build.sh`. It guards against double-injection via `Symbol.for('pod.runtime')` and shows a visual overlay indicator.
...
index.html
  └─ clawser-app.js
       ...
       │    └─ packages/pod/ (Pod base class, discovery, messaging)
...
graph TD
    ...
    POD --> POD_PKG["packages/pod/ (Pod base)"]
    POD --> MESH["mesh modules"]
